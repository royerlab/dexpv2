...
#   https://github.com/czbiohub/mantis/blob/main/mantis/analysis/deskew.py
...
def deskew(
    ...
    Parameters
    ----------
    raw_data : ArrayLike with ndim == 3
    ...
    Returns
    -------
    deskewed_data : ArrayLike with ndim == 3
    ...
    if cval is None:
        cval = raw_data.min().item()

    # Prepare transforms
    Z, Y, X = raw_data.shape

    ct = np.cos(np.deg2rad(ls_angle_deg))
    Z_shift = 0
    ...
    matrix = np.asarray(
        [
            [-px_to_scan_ratio * ct, 0, px_to_scan_ratio, Z_shift],
            [-1, 0, 0, Y - 1],
            [0, -1, 0, X - 1],
        ],
        like=raw_data,
    ...
    # Apply transforms
    deskewed_data = ndi.affine_transform(
        raw_data,
        matrix,
